...
bool DXApplication::initialize(HWND hWnd, int width, int height)
{
	...
	if (!loadFullScreenQuad())
		return false;
	if (!loadTexture(L"data/fiesta_converted.dds", &m_srcTexture))
		return false;
...
bool DXApplication::runComputeShader(LPCWSTR shaderFilename)
{
	...
	D3D11_TEXTURE2D_DESC sourceTexDesc;
	m_srcTexture->GetDesc(&sourceTexDesc);
	viewDesc.Format = sourceTexDesc.Format; //DXGI_FORMAT_R8G8B8A8_UNORM;
	viewDesc.ViewDimension = D3D_SRV_DIMENSION_TEXTURE2D;
	viewDesc.Texture2D.MipLevels = 1;
	viewDesc.Texture2D.MostDetailedMip = 0;
	HRESULT hr = m_pd3dDevice->CreateShaderResourceView(m_destTexture, &viewDesc, &m_destTextureView);
	if(FAILED(hr))
	{
...
bool DXApplication::loadFullScreenQuad()
{
	...
	if (FAILED(D3DCompileFromFile(L"./data/fullQuad.fx", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "VS", "vs_4_0", dwShaderFlags, D3DCOMPILE_EFFECT_ALLOW_SLOW_OPS, &pVSBlob, &pErrorBlob)))
	{
		if (pErrorBlob != NULL)
		{
	...
	if (FAILED(D3DCompileFromFile(L"./data/fullQuad.fx", NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "PS", "ps_4_0", dwShaderFlags, D3DCOMPILE_EFFECT_ALLOW_SLOW_OPS, &pPSBlob, &pErrorBlob)))
	{
		if (pErrorBlob != NULL)
		{
			OutputDebugStringA((char*)pErrorBlob->GetBufferPointer());
...
bool DXApplication::loadTexture(LPCWSTR filename, ID3D11Texture2D** texture)
{
	...
	//Included header and source file for texture loader from directxtk .
	DirectX::DDS_ALPHA_MODE mode;

	//only works for dds - need to add my own file load + convert utility in future.
	HRESULT hr = DirectX::CreateDDSTextureFromFile(m_pd3dDevice, filename, (ID3D11Resource **)texture, &m_srcTextureView,0, &mode);
	if (hr == S_OK)
	{
		D3D11_TEXTURE2D_DESC desc;
...
bool DXApplication::createInputBuffer()
{
	...
	if (m_srcTextureData)
	{
		...
		descGPUBuffer.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED; //| D3D11_RESOURCE_MISC_BUFFER_ALLOW_RAW_VIEWS;
		...
		HRESULT hr = m_pd3dDevice->CreateBuffer(&descGPUBuffer, &InitData, &m_srcDataGPUBuffer);
		if (FAILED(hr))
			return false;
...
bool DXApplication::loadComputeShader(LPCWSTR filename, ID3D11ComputeShader** computeShader)
{
	...
	ID3DBlob* pBlob = NULL;
	HRESULT hr = D3DCompileFromFile(filename, NULL, D3D_COMPILE_STANDARD_FILE_INCLUDE, "CSMain", pProfile, dwShaderFlags, 0, &pBlob, &pErrorBlob);
	if (FAILED(hr))
	{
		if (pErrorBlob)
			OutputDebugStringA((char*)pErrorBlob->GetBufferPointer());
